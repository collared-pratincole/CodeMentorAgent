...
import {
  Key, Cpu, BookOpen, Palette, Plus, Trash2,
  CheckCircle2, XCircle, Loader2, RotateCcw, Save, Terminal, Pencil,
  Users, UserPlus, LogIn, LogOut, Zap
} from 'lucide-react'
...
import { detectLanguage, getAvailableLanguages } from '@/services/api'
...
// ==================== API 配置 Tab ====================
function APITab() {
  const { userModels, addUserModel, removeUserModel, updateUserModel, saveModelApiKey, loadApiKeyPreviews } = useSettingsStore()
  const [showAdd, setShowAdd] = useState(false)

  // 进入设置页时加载所有模型的 apiKey 脱敏预览
  useEffect(() => {
    void loadApiKeyPreviews()
  }, [loadApiKeyPreviews])

  // AddModelModal 的回调：先 addUserModel（不含 apiKey），再 saveModelApiKey 到后端
  const handleAddWithKey = async (model: Omit<UserModel, 'id'>, apiKey: string) => {
    const id = addUserModel(model)
    if (apiKey) {
      await saveModelApiKey(id, apiKey)
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <AnimatePresence>
        {showAdd && (
          <AddModelModal onClose={() => setShowAdd(false)} onAdd={handleAddWithKey} />
        )}
      </AnimatePresence>
    </div>
  )
...
function ModelCard({ model, onRemove, onUpdate }: {
  ...
  const { saveModelApiKey } = useSettingsStore()
  ...
  const handleTest = async () => {
    ...
    setTesting(true)
    setTestResult(null)
    // 通过后端代理测试（apiKey 已存后端，前端不持有明文）
    const ok = await testConnection(model.baseUrl, model.model, {
  ...
  const handleSaveKey = async () => {
    ...
    setSavingKey(true)
    try {
      const ok = await saveModelApiKey(model.id, newKey.trim())
      if (ok) {
        setEditingKey(false)
        setNewKey('')
      }
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                title={!model.hasApiKey ? '请先录入 API Key' : ''}
              >
                ...
                {editingKey ? (
                  <>
                    <input
                      ...
                      autoFocus
                      className="flex-1 rounded-lg border border-cm-accent/40 bg-cm-bg px-3 py-1.5 text-xs font-mono text-cm-text placeholder:text-cm-muted/50 focus:border-cm-accent focus:outline-none"
                    />
                    <button
                      onClick={handleSaveKey}
                      disabled={savingKey || !newKey.trim()}
                      className="rounded-lg bg-cm-accent px-2.5 py-1.5 text-xs font-medium text-white hover:opacity-90 disabled:opacity-40"
                    >
                  ...
                  <>
                    <code className="flex-1 rounded-lg bg-cm-card-alt px-3 py-1.5 text-xs font-mono text-cm-text-secondary truncate">
                      {model.hasApiKey ? (model.apiKeyPreview || '已配置') : '未配置'}
                    </code>
                    <button
                      onClick={() => setEditingKey(true)}
                      className="rounded-lg p-1.5 text-cm-muted hover:text-cm-accent transition-colors"
                      title={model.hasApiKey ? '更新 API Key' : '录入 API Key'}
                    >
...
function AddModelModal({ onClose, onAdd }: {
  onClose: () => void
  // 安全改动：onAdd 现在接收 (model, apiKey)，apiKey 由回调保存到后端
  onAdd: (model: Omit<UserModel, 'id'>, apiKey: string) => Promise<void>
}) {
            ...
            >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-cm-border bg-cm-bg px-4 py-2.5 text-sm font-mono text-cm-text placeholder:text-cm-muted/50 focus:border-cm-accent focus:outline-none focus:ring-1 focus:ring-cm-accent/30"
                />
                <p className="mt-1 text-xs text-cm-muted">密钥将保存到后端，前端仅显示脱敏预览（如 sk-***...1234）</p>
              </div>
...
function EnvironmentTab() {
  ...
  // 首次进入环境标签页时自动扫描一次（仅当从未扫描过且列表为空时）
  useEffect(() => {
    if (hasAutoDetected || envConfigs.length > 0) return
    setHasAutoDetected(true)
    handleAutoDetect(true)
  }, [hasAutoDetected, envConfigs.length])

  // 自动扫描本地可用语言并自动添加
  // silent 参数：首次自动扫描不显示"扫描失败"等提示
  const handleAutoDetect = async (silent = false) => {
    setAutoDetecting(true)
    setAutoDetectResult(null)
    try {
      const available = await getAvailableLanguages()
      const langIds = Object.keys(available)
      if (langIds.length === 0) {
        setAutoDetectResult('未检测到任何本地语言环境')
        return
      }
      ...
      for (const langId of langIds) {
        const version = available[langId]
        const cmds = LANG_COMMANDS[langId]
        const langInfo = LANGUAGES.find(l => l.id === langId)
        const label = langInfo?.name || langId

        // 检查是否已存在同语言的配置
        const existing = envConfigs.find(e => e.languageId === langId)
        if (existing) {
          updateEnvConfig(existing.id, { status: 'installed', version, installPath: cmds?.run || '' })
          updated++
        } else {
          addEnvConfig({
            languageId: langId,
            label,
            version,
            installPath: cmds?.run || '',
            detectCommand: cmds?.detect || `${langId} --version`,
            runCommand: cmds?.run || langId,
      ...
      setAutoDetectResult(`扫描完成：新增 ${added} 个，更新 ${updated} 个`)
    } catch {
      if (!silent) setAutoDetectResult('扫描失败，请检查后端服务是否运行')
    }
    setAutoDetecting(false)
    if (!silent) setTimeout(() => setAutoDetectResult(null), 6000)
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-2 rounded-xl border border-cm-accent/30 bg-cm-accent-light px-4 py-2.5 text-sm font-medium text-cm-accent transition-colors hover:bg-cm-accent/20 disabled:opacity-50"
          >
            {autoDetecting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Zap className="h-4 w-4" />}
            自动扫描
          </button>
          <button
            onClick={() => setShowAdd(true)}
      ...
      {autoDetectResult && (
        <div className="rounded-xl border border-cm-accent/30 bg-cm-accent-light px-4 py-2.5 text-sm text-cm-accent">
          {autoDetectResult}
        </div>
      )}
...
function EnvCard({ env, onRemove, onUpdate }: {
  ...
  const handleDetect = async () => {
    ...
    setDetectVersion(null)
    try {
      const result = await detectLanguage(env.languageId)
      if (result.installed) {
        setDetectResult('installed')
        setDetectVersion(result.version)
        // 同步更新 envConfig 的状态和版本
        onUpdate(env.id, { status: 'installed', version: result.version || env.version })
      } else {
        setDetectResult('not_found')
        onUpdate(env.id, { status: 'not_installed' })
      }
    } catch {
      setDetectResult('not_found')
    }
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {detectResult === 'installed' && (
                <span className="flex items-center gap-1 text-xs text-cm-green">
                  <CheckCircle2 className="h-3.5 w-3.5" /> 已安装{detectVersion ? ` · ${detectVersion}` : ''}
                </span>
              )}
