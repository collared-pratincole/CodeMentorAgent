...
export default function ModelSelector() {
  ...
  if (userModels.length === 0) {
    return (
      <a
        href="/settings"
        className="flex items-center gap-2 rounded-xl border border-cm-accent/30 bg-cm-accent-light px-2 py-1.5 text-sm text-cm-accent hover:bg-cm-accent-light/80 transition-colors lg:px-3"
      >
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        title={activeModel?.label || activeModel?.model || '选择模型'}
        className={cn(
          'flex items-center gap-2 rounded-xl border border-cm-border bg-cm-card px-2 py-1.5 text-sm transition-colors lg:px-3',
      ...
      >
        ...
        <span className="hidden lg:inline text-cm-text font-medium max-w-[120px] truncate">
          {activeModel?.label || activeModel?.model || '选择模型'}
        </span>
        <ChevronDown
          className={cn('hidden lg:block h-3.5 w-3.5 text-cm-muted transition-transform', open && 'rotate-180')}
        />
      </button>
